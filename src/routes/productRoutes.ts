...
const show = async (req: Request, res: Response) => {
  try {
    const product: Product | null = await productModel.show(
...
const update = async (req: Request, res: Response) => {
  try {
    const product: Product | null = await productModel.update(
      req.params.id as unknown as number,
      req.body.name,
      req.body.price,
      req.body.category,
...
const remove = async (req: Request, res: Response) => {
  try {
    const product: Product | null = await productModel.delete(
      req.params.id as unknown as number,
    );
    res.json(product);
  } catch (err) {
